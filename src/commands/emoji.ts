...
export const emojiCommand: Command = {
    ...
    async execute(interaction) {
        ...
        if (sub === 'add') {
            ...
            if (!name || name.length < 2) {
                const c = ComponentsV2.errorContainer('<:Cross:1524363088621469737> Invalid Name', 'Emoji name must be at least 2 characters (a-z, 0-9, underscore).');
                await interaction.reply({ components: [c], flags: V2 });
                return;
            }

            const c = ComponentsV2.infoContainer('<:Message:1524363100734623836> Add Emoji',
            ...
            const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
                ...
                new ButtonBuilder()
                    ...
                    .setLabel('From Reply')
                    .setEmoji('<:Retry:1524363041024512010>')
                    .setStyle(ButtonStyle.Secondary),
                new ButtonBuilder()
        ...
        if (sub === 'list') {
            ...
            if (emojis.size === 0) {
                const c = ComponentsV2.infoContainer('<:Message:1524363100734623836> Emoji List', 'No custom emojis in this server.');
                await interaction.reply({ components: [c], flags: V2 });
                return;
            }
            ...
            let text = `# <:Message:1524363100734623836> Custom Emojis (${emojis.size} total)\n\n`;
            if (static_.size > 0) {
        ...
        if (sub === 'delete') {
            ...
            if (!emoji) {
                const c = ComponentsV2.errorContainer('<:Cross:1524363088621469737> Emoji Not Found',
            ...
            try {
                await emoji.delete();
                const c = ComponentsV2.successContainer('<:Tick:1524363090626482326> Emoji Deleted',
                ...
                const c = ComponentsV2.errorContainer('<:Cross:1524363088621469737> Failed to Delete', err.message || 'Unknown error.');
                await interaction.editReply({ components: [c], flags: V2 });
            }
            return;
        }
        ...
        if (sub === 'rename') {
            ...
            if (!newName || newName.length < 2) {
                const c = ComponentsV2.errorContainer('<:Cross:1524363088621469737> Invalid Name', 'Name must be at least 2 characters (a-z, 0-9, underscore).');
                await interaction.reply({ components: [c], flags: V2 });
                return;
            }
            ...
            try {
                ...
                const c = ComponentsV2.successContainer('<:Tick:1524363090626482326> Emoji Renamed',
                    `Renamed **\`:${emoji.name}:\`** → **\`:${newName}:\`**`);
                await interaction.editReply({ components: [c], flags: V2 });
            } catch (err: any) {
                logger.error('Failed to rename emoji:', err);
                const c = ComponentsV2.errorContainer('<:Cross:1524363088621469737> Failed to Rename', err.message || 'Unknown error.');
                await interaction.editReply({ components: [c], flags: V2 });
            }
            return;
        }
    ...
    async handleButton(interaction) {
        ...
        if (action === 'cancel') {
            const c = ComponentsV2.infoContainer('<:Cross:1524363088621469737> Cancelled', 'Emoji creation cancelled.');
        ...
        if (action === 'upload') {
            ...
            const c = ComponentsV2.infoContainer('<:Message:1524363100734623836> Upload Image',
        ...
        if (action === 'reply') {
            ...
            if (!channel) {
                await interaction.update({ components: [ComponentsV2.errorContainer('<:Cross:1524363088621469737> No Channel', 'Could not find this channel.')] });
                return;
            }
            ...
            if (!userMsg || !userMsg.reference?.messageId) {
                const c = ComponentsV2.errorContainer('<:Cross:1524363088621469737> No Reply Found', 'Reply to a message containing an image, then click this button.\n\n**How to use:**\n1. Reply to a message that has an image\n2. Click "From Reply"');
                await interaction.update({ components: [c] });
                return;
            }
            ...
            if (!imageUrl) {
                const c = ComponentsV2.errorContainer('<:Cross:1524363088621469737> No Image Found', 'The message you replied to does not contain an image.');
                await interaction.update({ components: [c] });
                return;
            }
    ...
    async handleModal(interaction) {
        ...
        if (!url || (!url.startsWith('http://') && !url.startsWith('https://'))) {
            const c = ComponentsV2.errorContainer('<:Cross:1524363088621469737> Invalid URL', 'Please provide a valid image URL starting with http:// or https://');
...
async function createEmoji(interaction: any, name: string, imageUrl: string) {
    ...
    try {
        ...
        if (!response.ok) {
            const c = ComponentsV2.errorContainer('<:Cross:1524363088621469737> Fetch Failed', `Could not fetch the image (HTTP ${response.status}). Make sure the URL is accessible.`);
